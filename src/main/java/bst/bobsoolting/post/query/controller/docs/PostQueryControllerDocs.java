...
public interface PostQueryControllerDocs {
    ...
    @Operation(summary = "카테고리별 게시글 조회", description = "특정 카테고리에 속한 게시글을 조회합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공", content = @Content(schema = @Schema(implementation = PostDTO.class))),
            @ApiResponse(responseCode = "500", description = "서버 오류")
    })
    @GetMapping("/category")
    ResponseEntity<List<PostDTO>> getPostsByCategory(
            @Parameter(description = "카테고리명", required = true, example = "BOB") @RequestParam("category") String category
    );
